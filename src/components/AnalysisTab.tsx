...
export default function AnalysisTab({
  ...
  const barChartData = React.useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      const txYear = parseInt(parsed.year, 10);
      if (txYear !== currentYear) return;

      const txMonth = parseInt(parsed.month, 10);
      if (txMonth >= 1 && txMonth <= 12) {
        ...
        if (tipo === 'RECEITA' || tipo === 'RECEITAS' || cat === 'RECEITA' || cat === 'RECEITAS') {
          monthsData[txMonth - 1].receitas += val;
        } else {
          monthsData[txMonth - 1].despesas += val;
        }
      }
    });
  ...
  const barChartStats = React.useMemo(() => {
    let totalYearExpenses = 0;
    let maxExpense = 0;
    let maxExpenseMonth = '';
    let monthsWithData = 0;

    barChartData.forEach(m => {
      totalYearExpenses += m.despesas;
      if (m.despesas > maxExpense) {
        maxExpense = m.despesas;
        maxExpenseMonth = m.name;
      }
      if (m.despesas > 0) {
        monthsWithData++;
      }
    ...
    const averageExpense = monthsWithData > 0 ? totalYearExpenses / monthsWithData : 0;
  ...
  // Dica do Dia (IA Gemini) States & Logic
  interface DailyTipData {
    titulo: string;
    dica: string;
    categoria_foco: string;
    tipo_acao: string;
  ...
  const fetchDailyTip = React.useCallback(async (forceRefresh = false) => {
    const todayKey = new Date().toISOString().split('T')[0];
    const cachedDate = localStorage.getItem('wealthflow_daily_tip_date');
    const cachedData = localStorage.getItem('wealthflow_daily_tip_data');

    if (!forceRefresh && cachedDate === todayKey && cachedData) {
      try {
        setDailyTip(JSON.parse(cachedData));
        return;
      } catch (e) {
        console.warn("Failed to parse cached daily tip, fetching new one.", e);
      }
    }
    ...
    const categoryBreakdown = [
      { name: "Casa", total: metrics.casa },
      { name: "Consumo", total: metrics.consumo },
      { name: "Abastecimento", total: metrics.abastecimento },
      { name: "Pessoal", total: metrics.pessoal },
      { name: "Lazer", total: metrics.lazer },
      { name: "Taxas", total: metrics.taxas },
      { name: "Outros", total: metrics.outros }
    ].sort((a, b) => b.total - a.total);

    try {
      const summaryPayload = {
        mesAnalise: getMonthLabel(selectedMonth),
        receitasTotais: metrics.receitas,
        despesasTotais: metrics.totalDespesas,
        saldoLiquido: metrics.diferenca,
        statusCaixa: metrics.diferenca >= 0 ? 'Superávit' : 'Déficit',
        maioresCategoriasDespesa: categoryBreakdown.slice(0, 3)
      };

      const sampleTransactions = monthTransactions.slice(0, 12).map(t => ({
        data: t.data,
      ...
      const data: DailyTipData = await res.json();
      setDailyTip(data);
      localStorage.setItem('wealthflow_daily_tip_date', todayKey);
      localStorage.setItem('wealthflow_daily_tip_data', JSON.stringify(data));
    } catch (err) {
      console.error("Error fetching daily tip:", err);
      const fallbackTip: DailyTipData = {
        titulo: metrics.diferenca >= 0 ? "Potencialize seu Superávit" : "Otimização de Despesas Focada",
        dica: metrics.diferenca >= 0 
          ? `No mês de ${getMonthLabel(selectedMonth)}, seu saldo foi positivo em ${formatBRL(metrics.diferenca)}. Considere destinar esse excedente para sua Reserva de Emergência ou metas anuais.`
          : `No mês de ${getMonthLabel(selectedMonth)}, as despesas totais (${formatBRL(metrics.totalDespesas)}) superaram as receitas (${formatBRL(metrics.receitas)}). Analise os custos em ${categoryBreakdown[0]?.name || 'Consumo'} para reequilibrar o caixa.`,
        categoria_foco: categoryBreakdown[0]?.name || "Geral",
        tipo_acao: metrics.diferenca >= 0 ? "Motivacional" : "Otimização de Custos",
        acao_sugerida: metrics.diferenca >= 0 
          ? "Reserve ao menos 10% do saldo excedente para sua meta de investimentos." 
          : "Revise os gastos não essenciais desta semana e estabeleça um limite por categoria.",
        fallback: true
      };
      setDailyTip(fallbackTip);
      localStorage.setItem('wealthflow_daily_tip_date', todayKey);
      localStorage.setItem('wealthflow_daily_tip_data', JSON.stringify(fallbackTip));
    } finally {
      setLoadingTip(false);
    }
  }, [metrics, selectedMonth, monthTransactions]);
  ...
  return (
    ...
    >
      ...
      {/* Card: Dica do Dia (IA Gemini) */}
      <div className="bg-gradient-to-br from-slate-900 via-indigo-950/25 to-slate-900 border border-indigo-500/30 p-5 sm:p-6 rounded-3xl shadow-xl space-y-4 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center shadow-lg shadow-indigo-500/20 shrink-0">
              <span className="material-symbols-outlined text-xl font-bold animate-pulse">auto_awesome</span>
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-white font-display text-sm tracking-wide uppercase">
                  Dica do Dia • Inteligência Financeira
                </h3>
                <span className="px-2 py-0.5 rounded-full text-[9px] font-mono font-bold bg-indigo-500/15 border border-indigo-500/30 text-indigo-300 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-ping" />
                  Gemini IA
                </span>
              </div>
              <p className="text-[11px] text-slate-400 mt-0.5">
                Conselhos e recomendações de otimização de custos com base no seu histórico real de gastos.
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            id="btn-refresh-daily-tip"
            onClick={() => fetchDailyTip(true)}
            disabled={loadingTip}
            className="self-start sm:self-center px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-indigo-500/40 text-slate-200 hover:text-white text-xs font-mono font-bold transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50 active:scale-95 shadow-md shrink-0"
            title="Gerar nova dica personalizada com base no histórico atualizado"
          >
            <span className={`material-symbols-outlined text-sm ${loadingTip ? 'animate-spin text-indigo-400' : 'text-slate-400'}`}>
              refresh
            </span>
            {loadingTip ? 'Gerando Dica...' : 'Atualizar Dica'}
          </button>
        </div>

        {/* Tip Content */}
        {loadingTip ? (
          <div className="bg-slate-950/60 border border-slate-850 p-6 rounded-2xl flex flex-col items-center justify-center gap-3 text-center min-h-[110px]">
            <div className="w-8 h-8 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin" />
            <p className="text-xs text-indigo-300 font-mono animate-pulse">
              Analisando padrão de receitas, despesas e categorias no Gemini IA...
            </p>
          </div>
        ) : dailyTip ? (
          <div className="bg-slate-950/70 border border-slate-800/80 p-4 sm:p-5 rounded-2xl space-y-3.5 relative z-10 backdrop-blur-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-850 pb-2.5">
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-mono font-bold bg-amber-500/10 border border-amber-500/20 text-amber-300 uppercase">
                  {dailyTip.tipo_acao}
                </span>
                <span className="px-2.5 py-0.5 rounded-md text-[10px] font-mono font-bold bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 uppercase">
                  {dailyTip.categoria_foco}
                </span>
              </div>
            </div>
            ...
            {dailyTip.acao_sugerida && (
              <div className="bg-indigo-500/10 border border-indigo-500/25 p-3 rounded-xl flex items-start sm:items-center gap-2.5 text-xs text-indigo-200 font-sans">
                <span className="material-symbols-outlined text-indigo-400 text-base shrink-0 mt-0.5 sm:mt-0 font-bold">
                  task_alt
                </span>
                <div>
                  <strong className="font-bold text-indigo-300 mr-1.5 uppercase font-mono text-[10px] tracking-wider block sm:inline">
                    Ação Prática Recomendada:
                  </strong>
                  <span>{dailyTip.acao_sugerida}</span>
                </div>
              </div>
            )}
          </div>
        ) : (
          <div className="bg-slate-950/60 border border-slate-850 p-4 rounded-2xl flex items-center justify-between gap-3 text-xs text-slate-400">
            <span>Carregando sua Dica do Dia personalizada...</span>
            <button
              onClick={() => fetchDailyTip(true)}
              className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold text-xs font-mono cursor-pointer transition-all"
            >
              Gerar Dica
            </button>
          </div>
      ...
      {/* Gráfico de Despesas Mensais (Ano Atual) */}
      <div className="bg-slate-900 border border-slate-800/80 p-5 rounded-3xl space-y-4 animate-fade-in">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          <div className="flex items-center gap-2">
            <span className="material-symbols-outlined text-rose-400">bar_chart</span>
            <div>
              <h3 className="font-bold text-white font-display text-xs uppercase tracking-wider">Comparativo de Despesas Mensais</h3>
              <p className="text-[10px] text-slate-400 font-mono uppercase tracking-widest mt-0.5">Ano de {currentYear}</p>
            </div>
          </div>
          <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider bg-slate-950/60 px-2 py-1 rounded-lg border border-slate-850">
            Total Anual: <strong className="text-rose-400">{formatBRL(barChartStats.totalYearExpenses)}</strong>
          </span>
        </div>

        {/* Quick Stats Grid */}
        <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
          <div className="bg-slate-950/40 p-2 sm:p-3 rounded-2xl border border-slate-850/50 flex flex-col justify-between min-w-0">
            <span className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-wider font-mono truncate">Total Gastos</span>
            <span className="text-[10px] sm:text-xs font-extrabold text-slate-100 font-mono mt-0.5 truncate" title={formatBRL(barChartStats.totalYearExpenses)}>
              {formatBRL(barChartStats.totalYearExpenses)}
            </span>
          </div>
          <div className="bg-slate-950/40 p-2 sm:p-3 rounded-2xl border border-slate-850/50 flex flex-col justify-between min-w-0">
            <span className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-wider font-mono truncate">Média Mensal</span>
            <span className="text-[10px] sm:text-xs font-extrabold text-slate-100 font-mono mt-0.5 truncate" title={formatBRL(barChartStats.averageExpense)}>
              {formatBRL(barChartStats.averageExpense)}
            </span>
          </div>
          <div className="bg-slate-950/40 p-2 sm:p-3 rounded-2xl border border-slate-850/50 flex flex-col justify-between min-w-0">
            <span className="text-[8px] sm:text-[9px] font-bold text-slate-500 uppercase tracking-wider font-mono truncate">Pico Gasto</span>
            <span className="text-[10px] sm:text-xs font-extrabold text-rose-400 font-mono mt-0.5 truncate" title={`${barChartStats.maxExpenseMonth} (${formatBRL(barChartStats.maxExpense)})`}>
              {barChartStats.maxExpenseMonth} <span className="text-[8px] sm:text-[10px] font-normal text-rose-400/80">({formatBRL(barChartStats.maxExpense)})</span>
            </span>
          </div>
        </div>
            ...
            <BarChart data={barChartData} margin={{ top: 10, right: 15, left: -10, bottom: 0 }}>
              ...
              >
                {barChartData.map((entry, index) => {
                  const isPeak = entry.name === barChartStats.maxExpenseMonth && entry.despesas > 0;
                  return (
                    <Cell 
                      key={`cell-${index}`} 
                      stroke={isPeak ? '#f43f5e' : '#fda4af'} 
