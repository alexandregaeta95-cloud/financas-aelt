...
export default function DatabaseConsole({
  ...
  const handleWipeAndImportCSV = async () => {
    ...
    const confirmWipe = confirm(
      ...
      "Isso irá APAGAR COMPLETAMENTE todas as transações atuais no banco de dados local e na planilha e reescrevê-las a partir do CSV fornecido.\n\n" +
    ...
    try {
      ...
      setCsvImportProgress(`Gravando: Salvando ${parsedTransactions.length} novos registros no cache local...`);
      localStorage.setItem('wealthflow_transactions', JSON.stringify(parsedTransactions));
    ...
    try {
      ...
      if (activeTab === 'transactions') {
        setTransactions(prev => {
          const updated = prev.filter(t => String(t.id) !== docIdStr);
          localStorage.setItem('wealthflow_transactions', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'risk_zones') {
        setRiskZones(prev => {
          const updated = prev.filter(z => String(z.id) !== docIdStr);
          localStorage.setItem('wealthflow_riskzones', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'infractions') {
        setInfractions(prev => {
          const updated = prev.filter(i => String(i.id) !== docIdStr);
          localStorage.setItem('wealthflow_infractions', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'non_appealed') {
        setNonAppealed(prev => {
          const updated = prev.filter(n => String(n.id) !== docIdStr);
          localStorage.setItem('wealthflow_nonappealed', JSON.stringify(updated));
          return updated;
        });
      }

      showStatus(`Registro "${id}" deletado com sucesso do cache local!`);
  ...
  const handleSeedCollection = async () => {
    try {
      if (activeTab === 'transactions') {
        ...
        localStorage.setItem('wealthflow_transactions', JSON.stringify(initialTransactions));
        showStatus('Coleção "transactions" semeada com sucesso!');
      } else if (activeTab === 'risk_zones') {
        setRiskZones(initialRiskZones);
        localStorage.setItem('wealthflow_riskzones', JSON.stringify(initialRiskZones));
        showStatus('Coleção "risk_zones" semeada com sucesso!');
      } else if (activeTab === 'infractions') {
        setInfractions(initialInfractions);
        localStorage.setItem('wealthflow_infractions', JSON.stringify(initialInfractions));
        showStatus('Coleção "infractions" semeada com sucesso!');
      } else if (activeTab === 'non_appealed') {
        setNonAppealed(nonAppealedInfractions);
        localStorage.setItem('wealthflow_nonappealed', JSON.stringify(nonAppealedInfractions));
        showStatus('Coleção "non_appealed" semeada com sucesso!');
      } else if (activeTab === 'settings') {
        localStorage.setItem('wealthflow_avatar_url', avatarUrl);
  ...
  const handleWipeCollection = async () => {
    const confirmWipe = confirm(
      `⚠️ ATENÇÃO - OPERAÇÃO IRREVERSÍVEL:\n\n` +
      `Isso irá APAGAR COMPLETAMENTE todos os registros da coleção "${activeTab}" do cache local.\n\n` +
    ...
    try {
      ...
      if (activeTab === 'transactions') {
        ...
        setRiskZones([]);
        localStorage.removeItem('wealthflow_riskzones');
      } else if (activeTab === 'infractions') {
        setInfractions([]);
        localStorage.removeItem('wealthflow_infractions');
      } else if (activeTab === 'non_appealed') {
        setNonAppealed([]);
        localStorage.removeItem('wealthflow_nonappealed');
      }
    ...
    try {
      ...
      if (activeTab === 'transactions') {
        setTransactions(prev => {
          ...
          const updated = exists ? prev.map(t => String(t.id) === docIdStr ? parsed : t) : [parsed, ...prev];
          localStorage.setItem('wealthflow_transactions', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'risk_zones') {
        setRiskZones(prev => {
          ...
          const updated = exists ? prev.map(z => String(z.id) === docIdStr ? parsed : z) : [parsed, ...prev];
          localStorage.setItem('wealthflow_riskzones', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'infractions') {
        setInfractions(prev => {
          ...
          const updated = exists ? prev.map(i => String(i.id) === docIdStr ? parsed : i) : [parsed, ...prev];
          localStorage.setItem('wealthflow_infractions', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'non_appealed') {
        setNonAppealed(prev => {
          ...
          const updated = exists ? prev.map(n => String(n.id) === docIdStr ? parsed : n) : [parsed, ...prev];
          localStorage.setItem('wealthflow_nonappealed', JSON.stringify(updated));
          return updated;
        });
      } else if (activeTab === 'settings') {
        if (parsed.avatarUrl) {
          ...
          localStorage.setItem('wealthflow_avatar_url', parsed.avatarUrl);
        }
      }

      showStatus(`Registro "${finalId}" salvo com sucesso no banco de dados!`);
      setIsInserting(false);
      setEditingDoc(null);
    } catch (e: any) {
      alert(`Erro ao validar JSON ou salvar no banco de dados: ${e.message}`);
    }
  };
      ...
      <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Acesso direto ao banco de dados live (WealthFlow)</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Apagar todos os registros da coleção atual"
          >
      ...
      {isInserting ? (
        <div className="bg-slate-950 border border-slate-850 p-4 rounded-xl space-y-3 animate-fade-in">
          ...
          <div className="space-y-1">
            <span className="block text-[9px] font-bold text-slate-400 font-mono">ESTRUTURA JSON</span>
            <textarea
              rows={12}
              value={newDocJson}
                ...
                >
                  ...
                  {isSelected && (
                    <div className="bg-slate-900/40 border-t border-slate-900 p-3 space-y-2 font-mono">
                      <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Documento JSON Completo</span>
