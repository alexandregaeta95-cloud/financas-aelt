...
import RiskZoneMapComponent from '../modules/risco/components/RiskZoneMapComponent';
import { 
  Radio, 
  Navigation, 
  MapPin, 
...
export default function RiskZonesTab({ 
  ...
  return (
    <div className="space-y-6 animate-fade-in" id="risk-zones-panel">
      ...
      {/* Cadastro de Área de Risco e Controles Principais */}
      <div className="bg-slate-900/40 border border-slate-800 p-5 rounded-2xl space-y-4 text-left shadow-lg" id="control-panel-container">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-1">
          <div>
            ...
            <p className="text-[10px] text-slate-400">Cadastre e configure perímetros para receber avisos e manter seu trajeto protegido.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {onReindexRiskZones && (
              <button
                type="button"
                onClick={onReindexRiskZones}
                className="bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/25 px-3 py-2 rounded-xl text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer active:scale-95"
                title="Renumerar e reordenar todos os perímetros sequencialmente a partir de 1"
              >
                <span className="material-symbols-outlined text-[16px]">format_list_numbered</span>
                <span>Renumerar IDs</span>
              </button>
            )}
            
            <button
              type="button"
              id="btn-register-risk-zone"
              onClick={() => {
                if (!showAddForm) {
                  loadZoneDraft();
                }
        ...
        {/* Form fields that appear right below the button when clicked */}
        <AnimatePresence>
          {showAddForm && (
            <motion.div
            ...
            >
              <form onSubmit={handleAddSubmit} className="bg-slate-950 border border-slate-800 p-4 rounded-xl space-y-4 text-left mt-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-850">
                  <h4 className="font-bold text-white font-display text-xs uppercase tracking-wider">Novo Perímetro de Risco</h4>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-900 border border-slate-800 rounded-xl px-2.5 py-2.5 text-xs text-white focus:border-emerald-500 outline-none"
                    />
                  </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row justify-end gap-2">
                  ...
                  <button
                    ...
                    className="px-3.5 py-2 rounded-xl border border-slate-800 hover:bg-slate-900 text-slate-300 text-xs font-bold cursor-pointer"
                  >
                    Manter Rascunho
                  </button>
                  <button
        ...
        {/* Button 'Ativar Alta Prioridade' placed right below this button/fields */}
        <div className="pt-3 border-t border-slate-800/80 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className={`p-2.5 rounded-xl transition-all ${highPriorityBackground ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-850 text-slate-500'}`}>
              <Zap className={`w-5 h-5 ${highPriorityBackground ? 'animate-pulse text-emerald-400 font-bold' : 'text-slate-500'}`} />
            </div>
            <div className="space-y-1">
              <h4 className="text-xs font-bold text-white uppercase tracking-wider font-display flex items-center gap-1.5 font-sans">
                Modo Trânsito Sem Paradas (Alta Prioridade)
                {highPriorityBackground && (
                  <span className="text-[9px] bg-emerald-500/20 text-emerald-400 px-1.5 py-0.5 rounded-full font-mono normal-case tracking-normal animate-pulse">
                    Ativo em 2º Plano
                  </span>
                )}
              </h4>
              <p className="text-[10px] text-slate-400 leading-relaxed font-sans">
                Evita que o smartphone durma ou congele o GPS. Mantém a <strong>tela ligada</strong> (Wake Lock) e roda <strong>mídia silenciosa</strong> para emitir avisos de voz mesmo no bolso.
              </p>
          ...
          <button
            type="button"
            id="btn-high-priority"
            onClick={toggleHighPriorityBackground}
            className={`w-full xl:w-auto px-4 py-2.5 text-[10px] font-bold uppercase tracking-wide rounded-xl transition-all flex items-center justify-center gap-1.5 shadow cursor-pointer active:scale-95 ${
              highPriorityBackground
      ...
      {/* Interactive Map Component & Radar Display */}
      <section className="space-y-4" id="risk-zone-map-section">
        {/* HUD Info bar */}
        <div className="bg-slate-900/90 border border-slate-800 px-4 py-3 rounded-2xl flex justify-between items-center z-10 relative shadow-md">
        ...
        <RiskZoneMapComponent 
          ...
          radarMode={radarMode}
          currentAlertZone={currentAlertZone}
          onStartEditZone={handleStartEdit}
        />
      </section>
