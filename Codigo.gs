 ...
 * ==============================================================================
 *
 * Mapeamento Rigoroso de Colunas (Linha 1 da aba Abastecimentos e Lançamentos - A a X):
 * A: Id
 * B: Data
 * C: Descrição
 * D: Valor
 ...
 * M: Litros
 * N: Preço_Litro
 * O: Completou_O_Tanque
 * P: KM_Percorrido
 * Q: Média_(Km/L)
 * R: Veiculo
 * S: Descrição_Do_Veículo
 * T: Motorista
 * U: Nome_Posto
...
/**
 * Lê uma aba e converte-a em array de objetos associando cabeçalhos (Linha 1).
 * Converte qualquer Date em string "DD/MM/YYYY" para evitar erros com .split ou manipulação de texto.
 */
...
function fetchTransactionsFromSheet() {
  ...
  sheetsToRead.forEach(function(sheetName) {
    ...
    rawItems.forEach(function(item) {
      ...
      var rawTipo = String(item.Tipo || item.tipo || '').trim().toUpperCase();
      var rawCat = String(cat || '').trim().toUpperCase();
      var finalTipo = 'DESPESA';
      if (rawTipo === 'RECEITA' || rawCat === 'RECEITA' || sheetName === 'Receitas') {
        finalTipo = 'RECEITA';
      } else if (isAbastSheet || rawCat === 'ABASTECIMENTO') {
        finalTipo = 'DESPESA';
      } else if (rawTipo) {
        finalTipo = rawTipo;
      }
      ...
      allTx.push({
        ...
        tipo: finalTipo,
        Tipo: finalTipo,
        categoria: cat || (finalTipo === 'RECEITA' ? 'RECEITA' : 'OUTROS'),
        Categoria: cat || (finalTipo === 'RECEITA' ? 'RECEITA' : 'OUTROS'),
...
function upsertArrayToSheet(ss, sheetName, headers, items) {
  ...
  var rows = (items || []).map(function(item) {
    ...
    return sheetHeaders.map(function(header) {
      ...
      if (hNorm === 'Tipo' || hUpper === 'TIPO') {
        var tType = item.Tipo !== undefined ? item.Tipo : (item.tipo !== undefined ? item.tipo : '');
        var tCat = item.Categoria !== undefined ? item.Categoria : (item.categoria !== undefined ? item.categoria : '');
        if (String(tType).toUpperCase() === 'RECEITA' || String(tCat).toUpperCase() === 'RECEITA' || sheetName === 'Receitas') return 'RECEITA';
        if (String(tCat).toUpperCase() === 'ABASTECIMENTO' || sheetName === 'Abastecimentos' || sheetName === 'Abastecimento') return 'DESPESA';
        return tType || 'DESPESA';
      }
